...
## TODO: 동일 종목 동일일 복수 공시 처리
같은 날 한 종목에 여러 공시가 발생하는 경우, 현재는 공시별로 독립적으로 MarketReaction을 생성한다. 하지만 전일 종가 vs 당일 종가 비교 방식이므로 모든 공시의 시장 반응이 동일한 등락률을 갖게 되어, 개별 공시의 영향을 분리할 수 없다. 추후 다음 방안을 검토해야 한다:
- 분 단위 주가 데이터로 공시 전후 단기 반응 측정
- 복수 공시를 그룹핑하여 하나의 복합 반응으로 집계
- 공시 중요도(카테고리/감성 점수) 기반 가중치 부여
